...
template<typename T, typename Distance, typename Node>
inline void build_random_splitting_hyperplane(const vector<Node*>& nodes, int f, bool cosine, Node* p, Node* q, int iterations, int min, int max) {
...
struct Angular : Base {
  ...
  static inline bool side(const Node<S, T>* n, const T* y, int f, Random& random, double spill_tree_window) {
      T dot = margin(n, y, f);
      if (dot >= -spill_tree_window && dot <= spill_tree_window)
          // in buffer window
          return 0;
      else if (dot > spill_tree_window)
          // right
          return 1;
      else
          // left
  ...
  template<typename S, typename T, typename Random>
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations, int min, int max) {
    ...
    if (method == 3)
        build_random_splitting_hyperplane<T, Angular, Node<S, T> >(nodes, f,  false, p, q, iterations, min, max);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
...
struct DotProduct : Angular {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations, int min, int max) {
    ...
    if (method == 3)
      build_random_splitting_hyperplane<T, DotProduct, Node<S, T> >(nodes, f,  false, p, q, iterations, min, max);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
...
struct Minkowski : Base {
  ...
  template<typename S, typename T, typename Random>
  static inline int side(const Node<S, T>* n, const T* y, int f, Random& random, int spill_tree_window) {
    T dot = margin(n, y, f);
    if (dot >= -spill_tree_window && dot <= spill_tree_window)
        // in buffer window
        return 0;
...
struct Euclidean : Minkowski {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n, int method, int iterations, int min, int max) {
    ...
    if (method == 3)
        build_random_splitting_hyperplane<T, Euclidean, Node<S, T> >(nodes, f,  false, p, q, iterations, min, max);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
    Base::normalize<T, Node<S, T> >(n, f);
    n->a = 0.0;
...
protected:
  ...
  int _min_vector_value;
  int _max_vector_value;
  double _average_height;
  double _average_nodes_number;
  double _objects_size;

  double _spill_tree_window;
public:
  ...
  void load_json_config() {
      try {
          ...
          std::map<std::string, int> methodMap = {
                  ...
                  {"build_random_splitting_hyperplane", 3}
          };
          ...
          _min_vector_value = std::numeric_limits<int>::min();
          if (!config["min_vector_value"].is_null()) {
              _min_vector_value = config["min_vector_value"].get<int>();
          }

          _max_vector_value = std::numeric_limits<int>::max();
          if (!config["max_vector_value"].is_null()) {
              _max_vector_value = config["max_vector_value"].get<int>();
          }

          _spill_tree_window = 0.0;
          if (!config["spill_tree_window"].is_null()) {
              _spill_tree_window = config["spill_tree_window"].get<double>();
          }
...
protected:
  ...
  S _make_tree(const vector<S>& indices, bool is_root, Random& _random, ThreadedBuildPolicy& threaded_build_policy) {
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
      ...
      // TODO: лучше передавать map
      D::create_split(children, _f, _s, _random, m, _splitting_method, _method_iterations, _min_vector_value, _max_vector_value);
      ...
      for (size_t i = 0; i < indices.size(); i++) {
        ...
        if (n) {
          //TODO: Spill-Tree
          int side = D::side(m, n->v, _f, _random, _spill_tree_window);
          if (side == 0) {
    ...
    m->n_descendants = is_root ? _n_items : (S)indices.size();
    // TODO: увеличивать тут локальный счетчик высоты дерева 1 раз
