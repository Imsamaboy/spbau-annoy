...
int precision(int f = 40, int n = 1000000) {
    ...
    //******************************************************
    //Building the tree
    AnnoyIndex<int, double, Euclidean, Kiss32Random, AnnoyIndexSingleThreadedBuildPolicy> t = AnnoyIndex<int, double, Euclidean, Kiss32Random, AnnoyIndexSingleThreadedBuildPolicy>(
            f);
...
////        double *vector = (double *) malloc(f * sizeof(double));
////        for (int z = 0; z < f; ++z) {
////            vector[z] = (distribution(generator));
////        }
//        t.add_item(index, vector[index]);
//        std::cout << vector;
//        std::cout << "Loading objects ...\t object: " << index + 1 << "\tProgress:" << std::fixed << std::setprecision(2)
//                  << (double) index / (double) (n + 1) * 100 << "%\r";
//    }
    std::cout << std::endl;
...
//    int j = rand() % n;
//    std::cout << j << std::endl;
    t.get_nns_by_item(15, 6, -1, &closest, nullptr);
    /*
